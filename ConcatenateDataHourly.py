    ...
    for family in families:
         for session in sessions:
            ...
            if session==None:
                ...
                cgm_path = os.path.join(base_dir, family, "cgm_data_processed_hourly_features.csv")
                # Construct the file path for the CSV data for the current family and session
                epoch_path = os.path.join(base_dir, family, "sleep_epochs_processed_hourly_features.csv")
            else:
                # Construct the file path for the CSV data for the current family and session
                cgm_path = os.path.join(base_dir, family, session, "cgm_data_processed_hourly_features.csv")
                # Construct the file path for the CSV data for the current family and session
                epoch_path = os.path.join(base_dir, family, session, "sleep_epochs_processed_hourly_features.csv")
...
epochs_output_file = os.path.join(r"L:\LovbeskyttetMapper01\StenoSleepQCGM", "concatenated_hourly_epochs.csv")

# Specify the filename for the concatenated CGM data
cgm_output_file = os.path.join(r"L:\LovbeskyttetMapper01\StenoSleepQCGM", "concatenated_hourly_cgm.csv")
